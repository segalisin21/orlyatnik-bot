...
export const kb = {
  ...
  AFTER_PAYMENT_INSTRUCTION: 'После оплаты поздравь и попроси человека подтвердить, что оплатил: написать в чате «оплатил» или «оплатила» (это важно). Затем переведи в чат к Кристине (@krisis_pr). Чек попроси прислать сюда в бота (фото или документ).',
