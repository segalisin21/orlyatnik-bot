...
- **Лист «Участники»** (имя листа именно «Участники»). Колонки (первая строка — заголовки):
  - `user_id`, `username`, `chat_id`, `status`, `fio`, `city`, `dob`, `companions`, `phone`, `comment`, `shift`, `payment_proof_file_id`, `final_sent_at`, `updated_at`, `created_at`, `last_reminder_at`, `consent_at`, `yookassa_payment_id`, `event`
  - Колонка **event**: пусто или `orlyatnik` / `pizhamnik` — выбор мероприятия при /start.
- **Лист «Логи»**:
...
- **Пижамник:** после выбора «Пижамник» пользователь видит стартовый экран с кнопками «Узнать программу», «Условия и стоимость», «Забронировать место». Если забронировано 21 место — показывается сообщение о листе ожидания и кнопка «Записаться в лист ожидания» (статус WAITLIST). После отправки чека участнику дополнительно отправляется сообщение про остаток 4500 ₽ и срок возврата (из `AFTER_RECEIPT_MESSAGE`). Напоминание за 10 дней до мероприятия (остаток 4500 ₽) — cron 11 марта в 10:00 МСК.
