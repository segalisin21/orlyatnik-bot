# Орлятник 21+ / Пижамник — Telegram-бот

Один бот для двух мероприятий: **Орлятник 21+** (лагерь) и **Пижамник** (21–22 марта, дом за городом). При первом обращении пользователь выбирает мероприятие; дальше консультации, сбор анкеты, приём оплаты (чек), уведомление админа, выдача ссылки на чат. LLM-ассистент, CRM в Google Sheets, state-machine, деплой на Railway.
...
- **Лист «Пижамник»** — только участники Пижамника. Те же колонки, что и в «Участники». Создай в той же таблице лист с именем **Пижамник** и такой же первой строкой (заголовки). При выборе «Пижамник» в боте строка переносится из «Участники» в «Пижамник». Обратный перенос (Пижамник → Участники) не предусмотрен.
- **Лист «Логи»**:
  - `timestamp`, `user_id`, `status`, `direction`, `message_type`, `text_preview`, `raw_json`
- **Лист «Настройки»** (необязательно): настройки Орлятника без правки кода. Колонки: **A** = ключ, **B** = значение. Админ меняет их через бота: **Админ-меню → ⚙ Настройки → Орлятник** → выбор параметра → ввод нового значения. Ключи: `NEXT_SHIFT_TEXT`, `DEFAULT_SHIFT`, `PRICE`, `DEPOSIT`, `PAYMENT_SBER`, `LOCATION`, `WHAT_INCLUDED`, `WHAT_TO_TAKE`, `OBJECTION_*`, `MEDIA_CHANNEL`, `AFTER_PAYMENT_INSTRUCTION`, `FIELD_PROMPT_*` и др. Если листа нет или ключа нет — используются значения из кода.
- **Лист «Настройки Пижамник»** (необязательно): настройки Пижамника. Та же структура (A = ключ, B = значение). В админке: **⚙ Настройки → Пижамник**. Ключи: `START_MESSAGE`, `PROGRAM_TEXT`, `CONDITIONS_TEXT`, `PRICE`, `DEPOSIT`, `REMAINDER`, `PLACES_LIMIT`, `PAYMENT_SBER`, `AFTER_RECEIPT_MESSAGE`, `REFUND_DEADLINE_TEXT`, `PLACES_FULL_MESSAGE`, `WAITLIST_CONFIRMED_MESSAGE`, `CONSENT_PD_TEXT` и др.
- **Лист «Ответы»** создаётся ботом при первом обращении к хранилищу ответов. Колонки: `question_normalized`, `answer`, `updated_at`. Хранит до 500 пар «вопрос–ответ» для повторяющихся вопросов; ответы при необходимости оживляются через LLM.

Бот создаёт строки в «Участники» при первом обращении пользователя; «Логи» только дополняет. Смену можно выбирать кнопками: в анкете (поле «смена»), при запросе «покажи анкету» (кнопка «Изменить смену») и в переписке по фразам «какие смены», «выбрать смену» и т.п.
...
- **Выбор мероприятия:** при первом /start (если у участника не выбран event) бот показывает кнопки «Орлятник 21+» и «Пижамник». После выбора все тексты, цены, смены и логика берутся из конфига выбранного мероприятия (`getKb(event)`).
- **Состояния (FSM):** NEW → INFO → (FORM_FILLING | WAITLIST) → FORM_CONFIRM → WAIT_PAYMENT → PAYMENT_SENT → CONFIRMED. **WAITLIST** — только для Пижамника, когда 21 место занято и пользователь записался в лист ожидания.
...
- **Пижамник (по ТЗ):** даты 21–22 марта; стоимость 8500 ₽ (задаток 4000 ₽, остаток 4500 ₽); возврат до 14 марта; экраны «Узнать программу» и «Условия и стоимость»; лимит 21 место; при заполнении — лист ожидания (WAITLIST); после отправки чека — сообщение про остаток и возврат (`AFTER_RECEIPT_MESSAGE`); напоминание за 10 дней до начала (cron 11 марта). Данные Пижамника хранятся в отдельном листе «Пижамник»; при выборе мероприятия строка переносится из «Участники» в «Пижамник».
...
| **Факты для агента** (цены, даты, возражения, реквизиты) | `src/config.ts` | Объект `kb` (Орлятник) и `kbPizhamnik` (Пижамник). В рантайме `getKb(event)` возвращает нужный набор; для Пижамника лист «Настройки» не используется. |
...
## База клиентов из старых таблиц (прошлый год)

Если есть выгрузки прошлого сезона — два CSV: **чаты** (переписка с ботом) и **Орлятник** (участники, анкеты, смены), из них можно собрать одну таблицу «база клиентов» с категориями для рассылок.

**Скрипт:** `scripts/build-client-base.ts`

**Запуск** (из корня проекта, нужен Node.js):

```bash
npm run client-base
```

По умолчанию читает файлы из папки `Downloads` текущего пользователя:
- `чаты  - Лист1.csv` (колонки: аккаунт, сообщение)
- `Орлятник - Лист4.csv` (username в первой колонке, далее ФИО, город, смена и т.д.)

Можно передать свои пути:

```bash
npx ts-node scripts/build-client-base.ts "C:\путь\чаты.csv" "C:\путь\Орлятник.csv"
```

**Результат:** в корне проекта создаётся `client-base.csv` с колонками:
- `username`, `category`, `fio`, `city`, `dob`, `companions`, `phone`, `shift`, `comment`, `note1`, `note2`, `source_chaty`, `source_orlyatnik`

**Категории:**
- **participated** — был в таблице Орлятник и указана смена (18–20 июля, 8–10 августа и т.п.)
- **filled_anketa** — был в Орлятнике, заполнены ФИО/город/телефон, но смена «Не указана»
- **just_wrote** — только переписка в чатах или нет данных анкеты

Эту таблицу можно загрузить в Google Sheets (новый лист), фильтровать по `category` и использовать для ручной или будущей автоматической рассылки (например, «всем участвовавшим» или «всем, кто писал, но не доехал»).
